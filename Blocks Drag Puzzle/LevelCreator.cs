...
public class LevelCreator : MonoBehaviour
{
    ...
    [SerializeField] private SpawnedBlock _blockPrefab; // block piece
	...
	private void Update()
	{
		if (Input.GetMouseButtonDown(0)) // 좌클릭
        {
            ...
            EditorUtility.SetDirty(_level); // 에디터 실행중에 변한값을 저장
        }
    ...
    {
        isNewLevel = !(_rows == _level.Rows && _columns == _level.Columns); // (3,3)이 아니면 새로운레벨
        if (isNewLevel)
        {
            _level.Rows = _rows;
    ...
    private void SpawnGrid()
    {
        ...
        for (int i = 1; i < sprites.Count; i++) // 1부터
        {
