  ...
  using Photon.Deterministic;
	...
	public unsafe class SpawnSystem : SystemSignalsOnly, ISignalOnPlayerAdded
	{
		...
		private void PlacePlayerOnSpawnPosition(Frame frame, EntityRef playerEntityRef)
		{
			...
			else if (availableSpawnPoints.Count == 0 && usedSpawnPoints.Count != 0)
			{
				...
				usedSpawnPoints = frame.ResolveList(spawnPointManager->UsedSpawnPoints);
			}
		...
		{
			Log.Info("Player Connected at frame: " + f.Number);
