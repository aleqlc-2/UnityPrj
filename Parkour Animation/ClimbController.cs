		...
		else
		{
			...
			if (neighbour.connectionType == ConnectionType.Jump && Input.GetButton("Jump"))
			{
				...
				if (neighbour.direction.y == 1)
					StartCoroutine(JumpToLedge("HandHopUp", currentPoint.transform, 0.34f, 0.65f, handOffset: new Vector3(0.25f, 0.08f, 0.15f)));
				else if (neighbour.direction.y == -1)
					StartCoroutine(JumpToLedge("HandHopDown", currentPoint.transform, 0.31f, 0.65f, handOffset: new Vector3(0.25f, 0.1f, 0.13f)));
				else if (neighbour.direction.x == 1)
					StartCoroutine(JumpToLedge("HandHopRight", currentPoint.transform, 0.31f, 0.65f));
				else if (neighbour.direction.x == -1)
					StartCoroutine(JumpToLedge("HandHopLeft", currentPoint.transform, 0.31f, 0.65f));
			}
